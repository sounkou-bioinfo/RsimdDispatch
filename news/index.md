...
- Fix template generation: `.def` files (operations.def, signatures.def,
  backends.def) are now treated as text and receive prefix substitution
  when
  [`use_simd_dispatch()`](https://sounkou-bioinfo.github.io/RsimdDispatch/reference/simd_dispatch_template_path.md)
  deploys the scaffold — previously they were binary-copied, causing the
  generated configure to fail to compile.
- Fix prefix substitution: use boundary-aware regex (`\b`) for all four
  C-prefix patterns so that env-var names like `RSD_LOG_PREFIX` and
  shell variables like `SRC_DIR_PATH` are no longer mangled in generated
  packages.
- Fix
  [`simd_backend()`](https://sounkou-bioinfo.github.io/RsimdDispatch/reference/simd_backend.md)
  test: allow `"partial:<name>"` return value when an
  explicitly-selected backend implements only a subset of operations
  (e.g. SSE2/SSE41 implement `count_nonzero` but not `convolve1d`).
- Fix `sd_register_kernel_table()` sentinel: detect the canonical
  sentinel `{SD_OP_COUNT, SD_SIG_NONE, NULL}` explicitly; a `NULL`
  invoke on any other row is now an error rather than silently
...
- Update
  ...
  documentation to list `"partial:<name>"` as a possible
...
- Move the staged-kernel-only `kernel_common.h` helper from `src/` into
  `tools/simdDispatch/kernels/` and the copied template kernel
  directory, and update downstream-template docs for that layout.
...
- Split the staged-kernel registration ABI into
  `tools/simdDispatch/kernels/kernel_api.h` so backend kernels no longer
  include private dispatch headers from `src/`. Backend files now expose
  `SdKernelDef` tables consumed by the generic dispatch core, keeping
  operation names and operation-specific wrappers out of
...
- Switch dispatch from one function pointer per operation to a backend
  `SdKernelDef` operation table. Backend diagnostics now reuse the C
  backend metadata table, and the R setter no longer hardcodes backend
  names.
- Add a dispatched
...
- Move demo kernels from `src/` to `tools/simdDispatch/kernels/` so
  copied templates keep build-time kernel sources separate from ordinary
  R package `src/` files.
