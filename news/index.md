...
- Split the staged-kernel registration ABI into
  `tools/kernels/kernel_api.h` so backend kernels no longer include
  private dispatch headers from `src/`. Backend files now expose R-style
  `RsdKernelDef` tables consumed by the generic dispatch core, keeping
