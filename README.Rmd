...
AVX, AVX-512, and NEON kernel objects during configuration; link one shared
library and switch among compiled and CPU-supported implementations at runtime
...
prefixes. It does not add a runtime dependency on `RsimdDispatch`. Replace the demo `count_nonzero()` kernels under `tools/kernels/` with your own
scalar/SSE/AVX/NEON kernels. The generated `configure` script probes compiler
support, stages selected kernel objects in `src/rsd-kernels/`, and leaves `src/Makevars`
to link those objects with the baseline R API, CPU detection, and dispatcher.
