...
Pure-C runtime SIMD dispatch templates for R packages. Compile scalar, SSE,
AVX, AVX-512, and NEON C translation units separately; switch among the compiled
and CPU-supported implementations at runtime without unsafe overrides.
...
prefixes. It does not add a runtime dependency on `RsimdDispatch`. Replace the demo `count_nonzero()` kernels with your own scalar/SSE/AVX/NEON
kernels. Keep the R API, CPU detection, and dispatcher at baseline compiler
flags; only ISA-specific translation units get flags such as `-mavx2` or
