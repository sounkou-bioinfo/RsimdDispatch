...
`simd_set_backend()` rejects uncompiled or CPU-unsupported backends. Operation-level backend availability is reported in `simd_info()$operation_backends`, and the active resolved backend for each operation is reported in `simd_info()$operation_selected_backends`.
...
shared library and selected through a guarded resolved operation table.
