...
# Custom engine: compile a C source chunk against the local static library
# and run the resulting binary, capturing stdout.
knitr::knit_engines$set(c_run = function(options) {
  src  <- tempfile(fileext = ".c")
  bin  <- tempfile()
  writeLines(options$code, src)

  flags <- paste(
    "-O2 -std=c11",
    "-I.", "-Ikernels",
    src,
    "build/libsimd_dispatch.a",
    "-lm",
    "-o", bin
  )
  compile <- system2("cc", flags, stdout = TRUE, stderr = TRUE)
  status  <- attr(compile, "status"); if (is.null(status)) status <- 0L

  out <- if (!is.null(status) && status != 0L) {
    paste(c("### compile error", compile), collapse = "\n")
  } else {
    paste(system2(bin, stdout = TRUE, stderr = TRUE), collapse = "\n")
  }
...
Initialise the library, optionally select a backend, fill a call-frame struct,
and invoke:
...
`sd_dispatch_invoke` writes results into the call-frame struct; there is no
return value and no allocation. `sd_init_dispatch` is idempotent.
...
Wall-clock timings comparing the scalar backend against the best available
backend for each operation. Each measurement runs the operation 2000 times and
reports the median nanoseconds per call.
