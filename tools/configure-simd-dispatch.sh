#!/bin/sh
# Configure helper for R packages that compile the rsd core library and
# several SIMD kernels as staged object files selected at runtime.
# It checks compiler ISA support, writes src/config.h, compiles:
#   - tools/lib/*.c  → src/rsd-lib/*.o   (pure-C, no SIMD flags, no R headers)
#   - tools/kernels/*.c → src/rsd-kernels/*.o (ISA-specific, with SIMDe)
...
# Compile a core library source file (tools/lib/) into src/rsd-lib/.
...
# config.h (for RSD_HAVE_* macros) and kernel_api.h (via simd_dispatch.h).
...
if ! compile_lib "simd_dispatch.c" "simd_dispatch.o"; then
