...
info <- simd_info()
print(info[c("compiled_backends", "available_backends", "operation_backends", "operation_selected_backends", "simde_native_backends", "target_arch", "target_os")])
...
stopifnot("wasm_simd128" %in% info$operation_backends$count_nonzero)
stopifnot("wasm_simd128" %in% info$operation_backends$convolve1d)
